...
const INVITE_SUBJECT = "Invitación al Santuario ResiDate";
const INVITE_BODY = "Saludos,\n\nEstá cordialmente invitado a experimentar nuestro santuario. Por favor, visite nuestro registro para organizar su próxima estancia.\n\nSaludos cordiales,";
    ...
    const clients = slots
        ...
        .reduce((acc: any[], curr) => {
            ...
            if (!existing) {
                acc.push({
                    id: curr.id,
                    name: curr.clientName || "Huésped Invitado",
        ...
        .map(client => ({
            ...client,
            status: client.isVIP ? "VIP" : (client.hasFutureBooking ? "Activo" : "Inactivo")
        }));
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-700 relative">
            ...
            {isInviteOpen && (
                <div className="fixed inset-0 bg-navy/20 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white p-8 w-full max-w-md shadow-2xl border border-navy/10 animate-in zoom-in-95">
                        <h3 className="text-2xl font-serif text-navy mb-2">Invitar a un Huésped</h3>
                        <p className="text-slate mb-6">Envía una invitación exclusiva para unirse al registro.</p>
                        <form onSubmit={handleInvite} className="space-y-4">
                            <div>
                                <label className="text-xs uppercase tracking-widest font-bold text-navy opacity-50 block mb-2">Email del Huésped</label>
                                <input
                                    autoFocus
                                    type="email"
                                    required
                            ...
                            <div className="flex gap-4 pt-2">
                                <Button type="button" variant="ghost" onClick={() => setIsInviteOpen(false)} className="flex-1">Cancelar</Button>
                                <Button type="submit" className="flex-1">Enviar Invitación</Button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            <div className="flex flex-col md-flex-row md-items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-serif text-navy">Registro de Clientes</h1>
                    <p className="text-slate italic">Cultivando relaciones con los huéspedes más exigentes del mundo.</p>
                </div>
                            ...
                            {clients.length > 0 ? (
                                                ...
                                                <>
                                                    ...
                                                    <div className="absolute right-0 top-full mt-2 w-48 bg-white shadow-2xl border border-navy/10 z-20 animate-in fade-in zoom-in-95">
                                                        ...
                                                        >
                                                            ...
                                                            {client.isVIP ? 'Quitar Estado VIP' : 'Marcar como VIP'}
                                                        </button>
                                ...
                                <tr>
                                    <td colSpan={4} className="p-12 text-center text-slate italic opacity-50">
                                        No se ha encontrado actividad reciente de clientes.
                                    </td>
                                </tr>
                            )}
                        </tbody>
